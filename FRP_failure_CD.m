%FRP failure analysis considering complete degradation
...
E1 = [38.6e9, 38.6e9, 38.6e9, 38.6e9, 38.6e9, 38.6e9, 38.6e9, 38.6e9];    
E2 = [8.27e9, 8.27e9, 8.27e9, 8.27e9, 8.27e9, 8.27e9, 8.27e9, 8.27e9];    
nu12 = [0.28, 0.28, 0.28, 0.28, 0.28, 0.28, 0.28, 0.28];                  
G12 = [4.14e9, 4.14e9, 4.14e9, 4.14e9, 4.14e9, 4.14e9, 4.14e9, 4.14e9];   
%Calculating minor poissons ratio
for i = 1:n
    nu21(i) = (E2(i)/E1(i))*nu12(i);
end

S1Tu = [1062e6, 1062e6, 1062e6, 1062e6, 1062e6, 1062e6, 1062e6, 1062e6];
S1Cu = [610e6, 610e6, 610e6, 610e6, 610e6, 610e6, 610e6, 610e6];          
S2Tu = [31e6, 31e6, 31e6, 31e6, 31e6, 31e6, 31e6, 31e6];                  
S2Cu = [118e6, 118e6, 118e6, 118e6, 118e6, 118e6, 118e6, 118e6];        
T12u = [72e6, 72e6, 72e6, 72e6, 72e6, 72e6, 72e6, 72e6];                 

a1 = [8.6e-6, 8.6e-6, 8.6e-6, 8.6e-6, 8.6e-6, 8.6e-6, 8.6e-6, 8.6e-6];   
a2 = [22.1e-6, 22.1e-6, 22.1e-6, 22.1e-6, 22.1e-6, 22.1e-6, 22.1e-6, 22.1e-6];   
b1 = [0, 0, 0, 0, 0, 0, 0, 0];                                            
b2 = [0, 0, 0, 0, 0, 0, 0, 0];                                            
...
t = [0.125e-3; 0.125e-3; 0.125e-3; 0.125e-3; 0.125e-3; 0.125e-3; 0.125e-3; 0.125e-3];  
...
while (size(DL,1) < n)
    ...
    for i=1:n
        ...
        T = [c^2, s^2, 2*s*c;           
            s^2, c^2, -2*s*c;
            -s*c, s*c, c^2 - s^2];
        Em(:,i) = T*[Ex(1,i); Ex(2,i); Ex(3,i)/2];      
        Sm(:,i) = Q(:,:,i)*[Em(1,i); Em(2,i); 2*Em(3,i)];     
    end
